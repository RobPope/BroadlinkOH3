...
public class ModelMapper {
    ...
    public static ThingTypeUID getThingType(int model) {
        ...
        if (model == 0x51e3)
            return BroadlinkBindingConstants.THING_TYPE_BG1; // British General BG1 800/900 Series twin wall socket,
                                                             // TODO: Add SP4b hex
                                                             // (Also similar to SP4 so could be extended easily)
        if (model == 0x51e2) // British General UHC/U-01 Smart Plug
            return BroadlinkBindingConstants.THING_TYPE_U01;
        if (model == 0x2712)
